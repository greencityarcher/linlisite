...
*linli custom code
**/
if ( ! function_exists( 'linli_get_avatar' ) ) :
	/**
	 * Retrieve the avatar `<img>` tag for a user, email address, MD5 hash, comment, or post.
	 *
	 * @since 2.5.0
	 ...
	 * @return string|false `<img>` tag for the user's avatar. False on failure.
	 */
	function linli_get_avatar( $id_or_email, $size = 96, $default = '', $alt = '', $args = null ) {
		$defaults = array(
			// get_avatar_data() args.
			'size'          => 96,
			'height'        => 125,
		...
		if ( wp_lazy_loading_enabled( 'img', 'linli_get_avatar' ) ) {
			$defaults['loading'] = 'lazy';
		}
		...
		if ( ! is_null( $avatar ) ) {
			/** This filter is documented in wp-includes/pluggable.php */
			return apply_filters( 'linli_get_avatar', $avatar, $id_or_email, $args['size'], $args['default'], $args['alt'], $args );
		}
		...
		$avatar = sprintf(
			"<img alt='%s' src='%s' srcset='%s' class='linli-avatar' height='%d' width='%d' %s/>",
			esc_attr( $args['alt'] ),
			esc_url( $url ),
			esc_url( $url2x ) . ' 2x',
			esc_attr( implode( ' ', $class ) ),
