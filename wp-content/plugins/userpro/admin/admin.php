...
class userpro_admin {
	...
	function load_services(){
			echo "Redirecting to Services , please wait...";
		?>
		<script>
			window.location = "http://demo.userproplugin.com/services/";
		</script>
	<?php 	
	}	
...
{		
   ...
   $filetype=array('text/csv','application/csv','application/excel','application/vnd.ms-excel','application/vnd.msexcel','text/anytext');
		if(	isset(	$_FILES['import_users_file']['tmp_name'] ) ){
		
			$filename = $_FILES['import_users_file']['tmp_name'];
			if (!in_array($_FILES['import_users_file']['type'], $filetype))
			{
			...
			$this->process_csv( $filename, $send_new_user_notification );
