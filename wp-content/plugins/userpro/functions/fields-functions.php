			...
			switch($array['type']) {
				...
				case 'radio-full':
					...
					foreach($array['options'] as $k=>$v) {
						...
						if (checked( $v, $value, 0 )) { $res .= ' class="checked"'; }
						$res .= '></span><input type="radio" data-condition_rule="'.$array['condition_rule'].'" data-condition_fields="'.$array['condition_fields'].'" data-condition_value="'.$array['conditiona_value'].'"  data-add_condition="'.$array['add_condition'].'"  value="'.$v.'" name="'.$key.'-'.$i.'" ';
						$res .= checked( $v, $value, 0 );
						$res .= " />$v</label>";
					}
